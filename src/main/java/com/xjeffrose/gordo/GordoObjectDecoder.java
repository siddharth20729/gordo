...
public class GordoObjectDecoder extends ByteToMessageDecoder {
  private static final Logger log = LoggerFactory.getLogger(GordoObjectDecoder.class.getName());


  public GordoMessage decode(byte[] msg) {
  ...
  private GordoMessage _decode(ByteBuf msg) {
    ...
    DefaultGordoMessage _msg = null;

    try {
      _msg = new DefaultGordoMessage(UUID.fromString(new String(id)), Op.fromInt(Ints.fromByteArray(op)), colFam, key, val);
    } catch (IllegalArgumentException e) {
      //      log.error("Failure during Decode: ", e);
      _msg = new DefaultGordoMessage(null, Op.fromInt(Ints.fromByteArray(op)), colFam, key, val);
    }
    ...
    if (hashCode.equals(messageHash)) {
      ...
      _msg.setDecoderResult(DecoderResult.failure(new GordoDecodeException()));
    }
