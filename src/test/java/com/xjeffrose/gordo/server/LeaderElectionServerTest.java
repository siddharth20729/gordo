...
public class LeaderElectionServerTest {
  ...
  public void castBadBallot() throws Exception {
    ...
    ChannelHandler testHandler = new GordoLeaderElectionHandler(2, campaignManager);
    ChannelHandler testHandler1 = new GordoLeaderElectionHandler(2, campaignManager);

    EmbeddedChannel ch1 = new EmbeddedChannel(testHandler);
    EmbeddedChannel ch2 = new EmbeddedChannel(testHandler1);

    ByteBuf req1 = Unpooled.buffer();

    req1.writeBytes(Ints.toByteArray(1));
    req1.writeBytes(Ints.toByteArray(2));
    ...
    ch1.writeInbound(req1);
    ch2.writeInbound(req2);
